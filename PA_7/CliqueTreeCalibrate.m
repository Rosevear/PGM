...
function [P, logZ] = CliqueTreeCalibrate(P, isMax)
    ...
    if (doLogZ)
        %%% YOUR CODE HERE:
        rootCliqueBeliefs = P.cliqueList(lastCliqueTwo);
        for i = 1:length(unnormalizedMessages)
            if ~isempty(unnormalizedMessages(i, lastCliqueTwo))
                rootCliqueBeliefs = FactorProduct(rootCliqueBeliefs, unnormalizedMessages(i, lastCliqueTwo));
            end
        end
        normalizationConstant = sum(rootCliqueBeliefs.val);
        logZ = log(normalizationConstant);
