...
function allAcc = LRSearchLambdaSGD(Xtrain, Ytrain, Xvalidation, Yvalidation, lambdas)
  ...
  for i = 1:length(lambdas)
   LRModelParams = LRTrainSGD(Xtrain, Ytrain, lambdas(i));
   LRModelPredictions = LRPredict(Xvalidation, LRModelParams);
   LRModelAccuracy = LRAccuracy(LRModelPredictions, Yvalidation);
   allAcc(i) = LRModelAccuracy;
  end
