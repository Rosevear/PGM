...
for i = 1:(n - 2)
    ...
    %Initialize the values for the factor to 1
    for j = 1:size(assignments, 1)
        curAssignment = assignments(j, :);
        factors(i) = SetValueOfAssignment(factors(i), curAssignment, 1);
    end
    
    for j = 1:length(tripletList)
        %disp(tripletList(j));
        curAssignment = tripletList(j).chars;
        %disp(curAssignment);
        curFactorValue = tripletList(j).factorVal;
