          ...
          elseif strcmp(TransName, 'MHSwendsenWang2')
              ...
              %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
               ...
               for k = 1:edge_factor.card(1)
                   new_numerator_value = GetValueOfAssignment(edge_factor, [k k]);
                   numerator = [numerator; new_numerator_value];
               end
               denominator = edge_factor.val(:);
               q_ij = sum(numerator) / sum(denominator);
...
for i = 1:max_iter
    ...
    newMarkovState = Trans(all_samples(i, :), G, F);
    all_samples(i + 1, :) = newMarkovState;
