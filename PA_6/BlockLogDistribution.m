...
%Find all of the factors which contain any of the variables we are block
%sampling
factorsWithVars = unique([G.var2factors{V}]);

assignments = repmat(A, d, 1);

%Modify the assignments to take into account the different values variables
%V might take on
assignments(:, V) = repmat(transpose(1:d), 1, length(V));

%Get the joint distribution of the relevant factors
jointDistribution = F(factorsWithVars(1));
for i = 2:length(factorsWithVars)
	jointDistribution = FactorProduct(jointDistribution, F(factorsWithVars(i)));
end

%Set the probability of selecting the current value for the block sample
for i = 1:d
	LogBS(i) = log(GetValueOfAssignment(jointDistribution, assignments(i, :), 1:length(A)));
end
