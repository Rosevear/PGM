...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
for i = 1:numPeople
    %Get the gene copy CPD's for the current person
    if pedigree.parents(i) == 0
        %The pedigree does not have the current person's parent's
        curGeneCopyOne = childCopyGivenFreqsFactor(alleleFreqs, i);
        curGeneCopyTwo = childCopyGivenFreqsFactor(alleleFreqs, numPeople + i);
    else
        %The pedigree does have the current person's parent's
        curParents = pedigree.parents(i, :);
        curGeneCopyOne = childCopyGivenParentalsFactor(numAlleles, i, curParents(1), numPeople + curParents(1));
        curGeneCopyTwo = childCopyGivenParentalsFactor(numAlleles, numPeople + i, curParents(2), numPeople + curParents(2));
    end
    factorList(i) = curGeneCopyOne;
    factorList(numPeople + i) = curGeneCopyTwo;
    
    %Get the phenotype CPD for the current person
    curPhenotypeFactor = phenotypeGivenCopiesFactor(alphaList, numAlleles, i, numPeople + i, (2 .* numPeople + i));
    factorList((2 .* numPeople) + i) = curPhenotypeFactor;
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  