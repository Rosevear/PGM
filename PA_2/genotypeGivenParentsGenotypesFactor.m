...
%the following code assumes that the current assignment for the genotype
%variables corresponds to the current genotype ID under consideration,
%which we can use in the genotypesToalleles Matrix
for i = 1:size(assignments, 1)
    curAssignment = assignments(i, :);
    curParentOneGenotype = genotypesToAlleles(curAssignment(2), :);
    curParentTwoGenotype = genotypesToAlleles(curAssignment(3), :);
    
    numGenotypes = 0;
    punnettSquare = {[curParentOneGenotype(1), curParentTwoGenotype(1)], [curParentOneGenotype(1), curParentTwoGenotype(2)], [curParentOneGenotype(2), curParentTwoGenotype(1)], [curParentOneGenotype(2), curParentTwoGenotype(2)]};
    for j = 1:length(punnettSquare)
        curAllele = punnettSquare{j};
        curPossibleGenotype = allelesToGenotypes(curAllele(1), curAllele(2));
        if curAssignment(1) == curPossibleGenotype
            numGenotypes = numGenotypes + 1;
        end
    end
    probabilityOfCurrentGenotype = numGenotypes ./ 4;
    genotypeFactor = SetValueOfAssignment(genotypeFactor, curAssignment, probabilityOfCurrentGenotype);    
end