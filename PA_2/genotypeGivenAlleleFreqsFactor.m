...
for i = 1:size(genotypesToAlleles, 1)
    curGenoTypeAlleleIDs = genotypesToAlleles(i, :);
    if curGenoTypeAlleleIDs(1) == curGenoTypeAlleleIDs(2)
        curGenotypeProbability = alleleFreqs(curGenoTypeAlleleIDs(1)) .* alleleFreqs(curGenoTypeAlleleIDs(2));
    else
        %To account for heterozygos genotype of the form Aa or aA
        curGenotypeProbability = 2 .* alleleFreqs(curGenoTypeAlleleIDs(1)) .* alleleFreqs(curGenoTypeAlleleIDs(2));
    end
    genotypeFactor = SetValueOfAssignment(genotypeFactor, i, curGenotypeProbability);
end
