...
%i iterates through the genotypes of all of the people, while numPeople + i
%iterates through the phenotypes
for i = 1:numPeople
    %Get the genotype CPD for the current person
    if pedigree.parents(i) == 0
        %The pedigree does not have the current person's parent's
        curGenotypeFactor = genotypeGivenAlleleFreqsFactor(alleleFreqs, i);
    else
        %The pedigree does have the current person's parent's
        curParents = pedigree.parents(i, :);
        curGenotypeFactor = genotypeGivenParentsGenotypesFactor(numAlleles, i, curParents(1), curParents(2));
    end
    factorList(i) = curGenotypeFactor;
    
    %Get the phenotype CPD for the current person
    curPhenotypeFactor = phenotypeGivenGenotypeFactor(alphaList, i, numPeople + i);
    factorList(numPeople + i) = curPhenotypeFactor;
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  