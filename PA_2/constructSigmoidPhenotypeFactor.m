...
for i = 1:size(assignments, 1)
    ...
    %This section could be generalized to accomodate more than 2 genes
    curGeneticEffect = alleleWeights{1}(curAssignment(2)) + alleleWeights{2}(curAssignment(3)) + alleleWeights{1}(curAssignment(4)) + alleleWeights{2}(curAssignment(5));
    curProbabilityOfTrait = computeSigmoid(curGeneticEffect);
    if curAssignment(1) == 1
        phenotypeFactor = SetValueOfAssignment(phenotypeFactor, curAssignment, curProbabilityOfTrait);
    else
        phenotypeFactor = SetValueOfAssignment(phenotypeFactor, curAssignment, 1 - curProbabilityOfTrait);
    end
end
