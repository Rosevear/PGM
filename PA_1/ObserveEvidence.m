...
for i = 1:size(E, 1),
    ...
    for j = 1:length(F),
        disp('NEW FACTOR CONSIDERED');
		  % Does factor contain variable?
        indx = find(F(j).var == v);

        if (~isempty(indx)),
            disp('FACTOR CONTAINS VARIABLE');
            ...
            for row = 1:size(assignments, 1)
                if assignments(row, indx) ~= x
                    F(j) = SetValueOfAssignment(F(j), assignments(row, :), 0);
                end
            end
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
