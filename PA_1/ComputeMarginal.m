...
if ~isempty(E)
    reducedJoint = ObserveEvidence(Joint, E);
    RenormalizingConstant = sum(reducedJoint.val);
    for i = 1:length(reducedJoint.val)
        reducedJoint.val(i) = reducedJoint.val(i) ./ RenormalizingConstant;
    end
    M = FactorMarginalization(reducedJoint, setdiff(reducedJoint.var, V));
else
